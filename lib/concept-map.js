var ConceptMap = function (chartElementId, infoElementId, dataJson) {
    ...
    var a = Math.max(window.innerWidth, 800), c = Math.max(window.innerHeight - 100, 800), h = c, U = 200, K = 22,
        S = 20, s = 8, R = 110, J = 30, o = 15, t = 10, w = 1400, F = "elastic", N = "#0da4d3";
    var T, q, x, j, H, A, P;
    var L = {}, k = {};
    var i, y;
    ...
    var B = d.append("g").attr("class", "links"), f = d.append("g").attr("class", "ditems"),
    ...
    function G(Y, X) {
        if (L.node === Y && X !== true) {
            if (Y.type === "ditem") {
                document.getElementById('popuptitle').innerText = Y.name;
                document.getElementById('popupdescription').innerText = Y.description;
                document.getElementById('popupprerequisites').innerText = Y.prerequisites;
                document.getElementById('popupsource').href = Y.source;
                document.getElementById('popupexample').innerText = Y.example;
                window.location.href = '#popup1';
                //change content window here
    ...
    function C(Z) {
        ...
        if (ab && ab.type === "ditem") {
            ...
            aa.append("text").attr("fill", N).attr("text-anchor", "middle").attr("y", (o + t) * -1).text(function (ae) {
                return "ATTACK " + ae.ditem
            })
        } else {
            if (ab && ab.type === "theme") {
                Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("ATTACK CLASS")
            } else {
